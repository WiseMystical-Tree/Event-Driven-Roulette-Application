...
def sincronizar_premios_com_ficheiro():
    ...
    for premio in premios_ficheiro:
        ...
        if existente is None:
            cor_importada = premio["cor"].strip() if premio["cor"] else ""
            if not cor_importada:
                cor_importada = gerar_cor_aleatoria()
...
class RolPremios:
    ...
    def __init__(self, root):
        ...
        self.canvas = tk.Canvas(root, width=520, height=520, bg="#0d0d0d", highlightthickness=0)
    ...
    def desenhar_roda(self):
        # Limpa todos os segmentos anteriores, mantendo a seta
        ...
        self.canvas.create_oval(
            self.centro_x - self.raio_roleta - 10,
            self.centro_y - self.raio_roleta - 10,
            self.centro_x + self.raio_roleta + 10,
            self.centro_y + self.raio_roleta + 10,
            fill="#111111",
            outline="#FFD700",
        ...
        for i in range(36):
            ...
            x = self.centro_x + (self.raio_roleta + 4) * math.cos(ang)
            y = self.centro_y - (self.raio_roleta + 4) * math.sin(ang)
            self.canvas.create_oval(
                x - 4,
                y - 4,
                x + 4,
        ...
        for i, premio in enumerate(self.premios):
            ...
            cor_fundo = premio["cor"].strip() or "#333333"
            if not cor_fundo.startswith("#") or len(cor_fundo) not in (4, 7):
                cor_fundo = "#333333"
            ...
            self.canvas.create_text(
                ...
                y,
                text=premio["nome"],
                fill=cor_texto_contraste(cor_fundo),
                font=("Verdana", 10, "bold"),
                width=90,
                justify="center",
                tags="segmento"
